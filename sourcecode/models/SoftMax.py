...
class SoftMax():
    ...
    """
    def __init__(self, sess, num_class=2, num_unit=100, learning_rate=0.01,weight_decay=0.000001,
                 size_doc_emd=300):
        """ 
    ...
    def modeling(self):
        ...
        """
        ## Placeholder
        self.pl_X_train = tf.placeholder(tf.float32, shape=[None, self.size_doc_emd])
        ...
        self.ln_w = tf.Variable(tf.truncated_normal([self.size_doc_emd, self.num_unit], stddev=1))
        self.ln_b = tf.Variable(tf.zeros([self.num_unit]))
        
        # trọng số và bias cho lớp tuyến tính phân loại cuối
        self.classify_w = tf.Variable(tf.truncated_normal([self.num_unit, self.num_class], stddev=1))
